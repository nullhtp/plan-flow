...
## Purpose
AI pipeline for goal understanding and board generation. Covers the LangGraph pipeline with nodes for goal classification (with confidence-based rejection), adaptive question generation (with follow-up rounds), DAG board generation (flat task list with dependency edges, convergence nodes, and goal node), and template generation from text/document content. All nodes use structured output enforcement via Pydantic schemas and OpenRouter LLM gateway.
## Requirements
