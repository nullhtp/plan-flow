...
**Reason**: Columns represented kanban phases. In the DAG model, task ordering comes from dependency edges, not columns. Tasks now belong directly to a Board.
**Migration**: Drop `board_column` table. Tasks gain a `board_id` FK replacing `column_id`.

### Requirement: Create Column Endpoint
**Reason**: No columns in DAG model.
**Migration**: Remove `POST /api/boards/:id/columns` endpoint.

### Requirement: Update Column Endpoint
**Reason**: No columns in DAG model.
**Migration**: Remove `PATCH /api/columns/:id` endpoint.

### Requirement: Delete Column Endpoint
**Reason**: No columns in DAG model.
**Migration**: Remove `DELETE /api/columns/:id` endpoint.
