...
**Reason**: Task-level action suggestions are replaced by subtask-level actions. The `POST /api/tasks/{task_id}/actions/suggest` endpoint is no longer needed.
**Migration**: Remove the endpoint from the AI router. Frontend no longer calls it.

### Requirement: Action Suggestion Schema
**Reason**: The `ActionSuggestion` and `ActionSuggestionsResponse` schemas are replaced by subtask-level schemas (`SubtaskActionOutput`, `SubtaskActionsResponse`).
**Migration**: Remove or replace with subtask action schemas.

### Requirement: Action Suggestion Prompt
**Reason**: The action suggestion prompt is repurposed for subtask-level action generation with a different structure and context.
**Migration**: Rewrite the prompt module for subtask actions.

### Requirement: Action Suggestion Model Configuration
**Reason**: The model configuration (`AI_ACTION_SUGGEST_MODEL`) is retained but now used for subtask action generation instead of task-level suggestions.
**Migration**: No change to the config key, only its usage context.
